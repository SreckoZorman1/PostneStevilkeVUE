<template>
    <div>
      ...
      <v-btn color="primary mb-3 mx-3" @click="getDrzaveCSV">
        Download CSV
      </v-btn>
      ...
      >
        ...
        <template v-slot:activator="{ on, attrs }">
            ...
            >
              ...
              <v-list>
                <router-link class="text-decoration-none" :to="`/admin/drzave/${item.id}/edit`">
                  <v-list-item>
...
export default {
  ...
  data() {
    return {
      ...
      modal: false,
      headers: [
        { text: 'drzava_lokalno', value: 'drzava_lokalno'},
        { text: 'oznaka_drzave', value: 'oznaka_drzave'},
        { text: 'drzava_iso', value: 'drzava_iso'},
        { text: 'opombe', value: 'opombe'},
        { text: 'Actions', value: 'actions', sortable: false }
      ],
      showFilters: false,
      filters: [{label: 'drzava_lokalno', title: 'drzava_lokalno'},{label: 'oznaka_drzave', title: 'oznaka_drzave'},{label: 'drzava_iso', title: 'drzava_iso'},{label: 'opombe', title: 'opombe'},
  ...
  methods: {
    ...
    async getDrzaveCSV() {
      const response = await axios({url: '/drzave?filetype=csv', method: 'GET',responseType: 'blob'});
      const type = response.headers['content-type']
      const blob = new Blob([response.data], { type: type, encoding: 'UTF-8' })
      const link = document.createElement('a')
      link.href = window.URL.createObjectURL(blob)
      link.download = 'drzaveCSV.csv'
      link.click()
    },
