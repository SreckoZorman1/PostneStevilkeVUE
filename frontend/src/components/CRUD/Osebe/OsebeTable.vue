<template>
    <div>
      ...
      <v-btn color="primary mb-3 mx-3" @click="getOsebeCSV">
        Download CSV
      </v-btn>
      ...
      >
            <template v-slot:top>
              ...
              <template v-slot:item.poste_stevilke_id="{ item }">
                <span>
                  {{ dataFormatter.postne_stevilkeOneListFormatter(item.poste_stevilke_id)}}
                </span>
              </template>
        ...
        <template v-slot:activator="{ on, attrs }">
            ...
            >
              ...
              <v-list>
                <router-link class="text-decoration-none" :to="`/admin/osebe/${item.id}/edit`">
                  <v-list-item>
...
export default {
  ...
  data() {
    return {
      ...
      showFilters: false,
      filters: [{label: 'ime', title: 'ime'},{label: 'priimek', title: 'priimek'},{label: 'emso', title: 'emso'},{label: 'spol', title: 'spol'},{label: 'naslov', title: 'naslov'},{label: 'enaslov', title: 'enaslov'},{label: 'telefon', title: 'telefon'},{label: 'davcna_stevilka', title: 'davcna_stevilka'},
  ...
  methods: {
    ...
    async getOsebeCSV() {
      const response = await axios({url: '/osebe?filetype=csv', method: 'GET',responseType: 'blob'});
