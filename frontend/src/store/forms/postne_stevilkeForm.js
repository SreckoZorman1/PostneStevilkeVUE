...
export default {
    ...
    actions: {
        async newHandler({commit, dispatch}, payload) {
            commit('startLoading')
            try {
                const result = await axios.post('/postne_stevilke', {data: payload.data})
                dispatch('snackbar/showSnackbar', 'Postne_stevilke has been created', {root: true})
        ...
        async edit({commit, dispatch}, payload) {
            commit('startLoading')
            try {
                const result = await axios.put(`/postne_stevilke/${payload.id}`, {id: payload.id, data: payload.data})

                dispatch('snackbar/showSnackbar', 'Postne_stevilke has been updated', {root: true})
                commit(`getData`, result.data)
            } catch (e) {
                dispatch('snackbar/showSnackbar', e, {root: true})
            } finally {
        ...
        try {
            if (val) {
                const result = await axios(`/drzave/autocomplete?query=${val}&limit=100`)
