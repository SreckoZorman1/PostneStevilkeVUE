  ...
  actions: {
    async newHandler({ commit, dispatch }, payload) {
      ...
      try {
        ...
        dispatch('snackbar/showSnackbar', 'Osebe has been created', {
          root: true,
        });
    ...
    async edit({ commit, dispatch }, payload) {
      commit('startLoading');
      try {
        const result = await axios.put(`/osebe/${payload.id}`, {
          id: payload.id,
          data: payload.data,
        });

        dispatch('snackbar/showSnackbar', 'Osebe has been updated', {
          root: true,
        });
        commit(`getData`, result.data);
      } catch (e) {
      ...
      try {
        if (val) {
          const result = await axios(
            `/postne_stevilke/autocomplete?query=${val}&limit=100`,
          );
