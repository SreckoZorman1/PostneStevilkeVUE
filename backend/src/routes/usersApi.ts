...
export default async (app: FastifyInstance) => {
    ...
    app.get<{
        ...
        const items = await repo.createQueryBuilder('item')
          .select(['item.id', 'item.phoneNumber'])

          .where("CAST(item.phoneNumber as TEXT) LIKE :query", { query: `%${req.query.query}%` })

          .orderBy('item.phoneNumber', 'ASC')
          .getMany();

        return items.map((item) => ({ id: item.id, label: toString(item.phoneNumber) }));
    });
